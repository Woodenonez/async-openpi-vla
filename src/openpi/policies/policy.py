...
class Policy(BasePolicy):
    ...
    @override
    def infer(self, obs: dict, *, noise: np.ndarray | None = None, nbatch=1) -> dict:  # type: ignore[misc]
        ...
        if self._is_pytorch_model:
            ...
            # device_get forces transfer from GPU/TPU to host
            vlm = np.asarray(jax.device_get(vlm[0, ...]), dtype=np.float32)
        outputs["vlm_fea"] = vlm
        return outputs
