...
from libero.envs.bddl_base_domain import BDDLBaseDomain
...
class ControlEnv:
    def __init__(
        ...
        **kwargs,
        ...
        old_part_cfg = suite.load_part_controller_config(default_controller=controller)
        controller_configs = refactor_composite_controller_config(
            controller_config=old_part_cfg,
            robot_type=robots[0],
            arms={"right": {}}
        )
        ...
        if TYPE_CHECKING:
            self.env = cast(BDDLBaseDomain, self.env)
